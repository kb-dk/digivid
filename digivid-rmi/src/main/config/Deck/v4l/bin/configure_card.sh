...
if [ ${FORMAT} = "1" ] || [ ${FORMAT} = "mpeg1" ]; then
   ...
   sleep .1
   ${IVCTL} -d ${DEVICE} -c stream_type=2,bitrate_mode=1,bitrate=1150000,bitrate_peak=1150000,audio=0xB8,framerate=25,dnr_mode=3,aspect=8
   sleep .1
   ${IVCTL} -d ${DEVICE} -y volume=25000
else
   ${IVCTL} -d ${DEVICE} -u 1 -f width=720,height=576
   sleep .1
   ${IVCTL} -d ${DEVICE} -c stream_type=10,audio=0xA9,bitrate_mode=1,bitrate=6500000,bitrate_peak=6500000,framerate=25,dnr_mode=3,aspect=1
   sleep .1
   ${IVCTL} -d ${DEVICE} -y volume=25000
fi
